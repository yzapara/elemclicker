...
namespace ElemClicker
{
    class Program
    {
        ...
        private static void LoginToGame(ChromeDriver driver, string login, string pass)
        {
            ...
            var inButton = driver.FindElementsByClassName("entrance");
        ...
        private static void RunUrfinRush(ChromeDriver driver)
        {
            var results = new List<string>(0) { "Started urfin rush at " + DateTime.Now };
            Console.WriteLine(string.Format("[{0}] Start urfin rush", DateTime.Now));

            try
            {
                var beatres = RunUrfinOnceRush(driver);
                if (beatres.Count == 0)
                {
        ...
        private static List<string> RunUrfinOnceRush(ChromeDriver driver)
        {
            ...
            try
            {
                #region Start battle
                {
                    ...
                    if (startDuelButton.Count > 0)
                    {
                        ...
                        {
                            driver.GetScreenshot().SaveAsFile(GetLogFileName("result[before]_", "jpg", "UrfinRush"), ImageFormat.Jpeg);
                        ...
                        if (startDuelButton.Count > 0)
                            ...
                            else if (startDuelButton.First().Text.StartsWith("Напасть сразу за"))
                            {
                    ...
                    if (cards.Count < 6)   //buttle is over
                    {
                        ...
                        if (firstButton.Count > 0)
                        {
                            if (firstButton.First().Text == "Далее")
                            {
                                ...
                                driver.GetScreenshot().SaveAsFile(GetLogFileName("result[after]_", "jpg", "UrfinRush"), ImageFormat.Jpeg);
                                firstButton.First().Click();

                            }                            
                        }
                        var label = driver.FindElementsByXPath("/html/body/div[3]/div[1]");
                        if (label.Count > 0)
                        {
                            if(label.FirstOrDefault().Text == "Армии Урфина разбиты!")
                            return result;
                        }
        ...
        private static List<string> RunUrfinOnce(ChromeDriver driver, bool noCharge, int startFrom = 10, bool changeCards = true, int maxReasonableDamage = -2500)
        {
            ...
            try
            {
                ...
                for (;;)
                {
                    ...
                    currentHealth = ConvertStrToDouble(healthStr);
        ...
        private static double ConvertStrToDouble(string healthStr)
        {
            try
            {
                if (healthStr.EndsWith("K"))
                    return Convert.ToDouble(healthStr.TrimEnd('K')) * 1000;
                else
                    return Convert.ToDouble(healthStr);
            }
            catch (Exception ex)
            {
        ...
        private static void RunDuels(ChromeDriver driver, double coeff)
        {
            ...
            results.AddRange(PickBonus(driver));
        ...
        private static List<string> PickBonus(ChromeDriver driver)
        {
            driver.Navigate().GoToUrl("http://elem.mobi/daily/");

            var result = new List<string>();

            try
            {
                var pickFirstBonusButton = driver.FindElementsByClassName("be");
                if (pickFirstBonusButton.Count > 0)
                    if (pickFirstBonusButton.First().Text == "Забрать награду")
                    {
                        pickFirstBonusButton.First().Click();
                        result.Add("Bonus's been picked");
                    }
            }
            catch (Exception e)
            {
        ...
        private static void BeatBoss(ChromeDriver driver, int bossNum)
        {
            ...
            if ((results.Count != 3) && (results[1] != "Boss not ready now"))
            {
        ...
        private static int GetRandCard(Random r)
        {
            var i = r.Next(1, 7);
            if (i == 2)
                return 1;
        ...
        private static string BeatBossOnce(ChromeDriver driver, int bossNum)
        {
            ...
            for (int i = 0; i < 20; i++)
            {
                try
                {
                    ...
                    if (cards.Count > 2)
                    {
                        var ii = GetRandCard(rnd);
                        Console.Write(string.Format("{0}, ", ii));
                        cards[ii].Click();
                    }
                    else
                    {
                        ...
                        if (resetButton.Count > 0)
                            ...
                            else
                            {
                                ...
                                else if (resetButton.First().Text == "Бить босса")
                                    return "Boss not ready now";
                                else
                                    return "WIN";
                            }
