    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            if (args.Length < 5)
            {
                ...
                //timeoutMsec = 3600000;
                //numrRepeats = 240;
                Console.WriteLine("Задайте параметры в такой последовательности: login password coeff numrRepeats timeout");
                Console.WriteLine("*login password - логин и пароль к игре");
                Console.WriteLine("*coeff максимальное превосходство противника при дуэлях в процентах.");
                Console.WriteLine("*numrRepeats - сколько раз программа будет выполняться после запуска");
                Console.WriteLine("*timeout - пауза в msec между запусками. Recommended value is 3600000 (1 час)");
                Console.Read();
            }
            else
            {
                ...
                {
                    Console.WriteLine("[{0}] Начало работы цикла {1}. Пауза между циклами {2} msc ", DateTime.Now, i, timeoutMsec);
                    ...
                    Console.WriteLine("[{0}] Конец работы цикла {1}. Пауза между циклами {2} msc ", DateTime.Now, i, timeoutMsec);
        ...
        private static string GetLogFileName(string pref, string end)
        {
            return string.Format(@"Logs\{0}_{1}_{2}_{3}_{4}_{5}_{6}_{7}.{8}",
                pref, DateTime.Now.Year, DateTime.Now.Month, DateTime.Now.Day, DateTime.Now.Hour, DateTime.Now.Minute, DateTime.Now.Second, DateTime.Now.Millisecond, end);
        }
        ...
        private static void RunDuels(ChromeDriver driver, double coeff)
        {
            ...
            Console.WriteLine(string.Format("[{0}] Начало дуэлей", DateTime.Now));
            ...
            string fileName = GetLogFileName("duel_", "txt");
                ...
                for (int i = 0; i < 20; i++)
                {
                    ...
                    if (startDuelButton.Count > 0)
                        if (startDuelButton.First().Text == "Напасть")
                        {
                            ...
                            if ((myHealth * coeff) >= oponnentHealth)
                            {
                                result.Add(string.Format("Select opponent with {0} health", oponnentHealth));
                                Console.WriteLine(string.Format("[{0}] Выбран оппонент для дуэли со здоровьем {1}", DateTime.Now, oponnentHealth));
                                startDuelButton.First().Click();
                                break;
                            }
                            result.Add(string.Format("Skip opponent with {0} health", oponnentHealth));
                            Console.WriteLine(string.Format("[{0}] Стремно выбирать для дуэли со здоровьем {1}.", DateTime.Now, oponnentHealth));
                        ...
                        if (startDuelButton.Count > 0)
                            if (startDuelButton.First().Text == "Еще дуэль")
                            {
                                ...
                                if (duelResult.Count > 0)
                                {
                                    ...
                                    {
                                        res = "— Поражение — " + res;
                                        driver.GetScreenshot().SaveAsFile(GetLogFileName("lostduel_", "jpg"), ImageFormat.Jpeg);
                                    }
                                    result.Add(string.Format("{0}", res));
                                    Console.WriteLine(string.Format("[{0}] Резульатат дуэли {1}", DateTime.Now, res));
                                }
                                ...
                                if (duelResult2.Count > 0)
                                {
                                    ...
                                    Console.WriteLine(string.Format("[{0}] Резульатат дуэли {1}", DateTime.Now, duelResult2.First().Text));
                                }
                                Console.WriteLine("=====================================================");
                                return result;
                            }
        ...
        private static void BeatBoss(ChromeDriver driver, int bossNum)
        {
            ...
            var results = new List<string>(0) { string.Format("Начал долбить босса {0} в {1}", bossNum, DateTime.Now) };
            Console.WriteLine(string.Format("[{0}] Начал долбить босса {1}", DateTime.Now, bossNum));
            ...
            for (var i = 0; i < numAttempts; i++)
            {
                ...
                {
                    if ((beatres == "WIN"))
                        driver.GetScreenshot().SaveAsFile(GetLogFileName(string.Format("boss{0}_", bossNum), "jpg"), ImageFormat.Jpeg);
                    break;
                }
            }

            results.Add(string.Format("Закончил долбить босса {0} за {1} попытки", DateTime.Now, results.Count - 2));
            Console.WriteLine(string.Format("[{0}] Закончил долбить босса {1} за {2} попытки ", DateTime.Now, bossNum, results.Count - 2));

            var fileName = GetLogFileName(string.Format("boss{0}_", bossNum), "txt");
            File.WriteAllLines(fileName, results);
        }
                        ...
                        if (resetButton.Count > 0)
                            if (resetButton.First().Text == "Начать снова")
                            {
                                ...
                                if (remainingHealth.Count > 0)
                                    return string.Format("Не смог за {0} ударов. Здоровье босса {1}", i, remainingHealth[1].Text);
                            }
                            else
                            {
